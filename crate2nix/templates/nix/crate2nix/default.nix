...
, buildRustCrateForPkgs ? if buildRustCrate != null then
    lib.warn
      "`buildRustCrate` is deprecated, use `buildRustCrateForPkgs` instead"
      (_: buildRustCrate)
  else
...
,
  ...
  */
  makeDefaultTarget = platform: {
    ...
    inherit (platform.rust.platform) arch os vendor;
    family = platform.rust.platform.target-family;
    env = "gnu";
    endian =
      if platform.parsed.cpu.significantByte.name == "littleEndian" then
        "little"
      else
        "big";
      ...
      let
        prefix =
          if builtins.stringLength crate == 1 then
            ...
            "${builtins.substring 0 2 crate}/${
            builtins.substring 2 (builtins.stringLength crate - 2) crate
          }";
      in
  ...
  */
  ...
  buildRustCrateWithFeatures =
    ...
    lib.makeOverridable
      ...
      in
      derivation // {
        passthru = (derivation.passthru or { }) // { testDrv = testDrv; };
      })
      {
  ...
  */
        ...
        buildByPackageIdForPkgsImpl = self: pkgs: packageId:
          let
            ...
            crateRenames =
              let
                grouped =
                  lib.groupBy (dependency: dependency.name) dependenciesWithRenames;
  ...
  dependencyDerivations = { buildByPackageId, features, dependencies, target, }:
    assert (builtins.isList features);
      ...
      expandFeaturesNoCycle = oldSeen: inputFeatures:
        if inputFeatures != [ ] then
          let
            ...
            # #209.
            enables = builtins.filter (f: !(seen ? "${f}"))
              (featureMap."${feature}" or [ ]);
          in
          [ feature ] ++ (expandFeaturesNoCycle seen
            (builtins.tail inputFeatures ++ enables))
      ...
      additionalDependencyFeatures =
        let
          ...
          stripPrefixMatch = prefix: s:
            if lib.hasPrefix prefix s then lib.removePrefix prefix s else null;
          ...
          dependencyFeatures =
            lib.filter (f: f != null) (map extractFeature features);
        in
        dependencyFeatures;
    in
